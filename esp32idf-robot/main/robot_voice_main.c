...
#define CONT_RNNM_AFE_VAD_CONFIRM_AVG 180
#define CONT_RNNM_AFE_VAD_CONFIRM_PEAK 850
#define CONT_RNNM_AFE_VAD_CONFIRM_MARGIN 60
...
#define CONT_RNNM_AFE_VAD_CONFIRM_HITS 3
#define CONT_RNNM_AFE_LOCAL_START_AVG 220
#define CONT_RNNM_AFE_LOCAL_START_PEAK 1100
#define CONT_RNNM_AFE_LOCAL_START_MARGIN 70
#define CONT_RNNM_AFE_LOCAL_START_HITS 3
...
static size_t s_barge_raw_upload_tail_discarded;
...
static size_t barge_raw_upload_discard_pending(void)
{
    size_t discarded = 0;
    portENTER_CRITICAL(&s_barge_raw_upload_mux);
    discarded = s_barge_raw_upload_size;
    s_barge_raw_upload_size = 0;
    portEXIT_CRITICAL(&s_barge_raw_upload_mux);
    s_barge_raw_upload_tail_discarded += discarded;
    return discarded;
}
...
static void raw_mic_upload_log_and_reset(uint32_t duration_ms, const char *reason, bool barge)
{
    ...
    size_t dropped = 0;
    size_t raw_tail_discard = s_barge_raw_upload_tail_discarded;
    uint32_t raw_tail_discard_ms = (uint32_t)(((raw_tail_discard / sizeof(int16_t)) * 1000U) / CONT_SR_VAD_RATE);
    portENTER_CRITICAL(&s_barge_raw_upload_mux);
    ...
    if (raw_upload) {
        ...
        if (!abort_upload) {
            if (barge_raw_upload && s_cont_barge_upload_skip_until_tick != 0 && now < s_cont_barge_upload_skip_until_tick) {
                ...
                if (discarded > 0) {
                    ESP_LOGI(TAG,
                             "barge upload raw tail dropped bytes=%u until=%ums",
                             (unsigned)discarded,
