...
def _openai_stream(api_base, api_key, messages, model, api_mode='chat_completions', *,
                   ...
                   max_tokens_field='max_tokens', top_p=None, frequency_penalty=None,
    ...
    if api_mode == "responses":
        ...
        if max_tokens: payload[max_tokens_field or "max_tokens"] = max_tokens
...
class BaseSession:
    def __init__(self, cfg):
        ...
        self.connect_timeout = max(1, int(cfg.get('connect_timeout', cfg.get('timeout', 5))))
        ...
        self.max_tokens = cfg.get('max_completion_tokens', cfg.get('max_tokens', 8192))
        default_max_tokens_field = 'max_completion_tokens' if 'mimo' in self.default_model.lower() else 'max_tokens'
        self.max_tokens_field = cfg.get('max_tokens_field', default_max_tokens_field)
        self.top_p = cfg.get('top_p')
        self.frequency_penalty = cfg.get('frequency_penalty')
...
class LLMSession(BaseSession):
    def raw_ask(self, messages):
        return (yield from _openai_stream(self.api_base, self.api_key, messages, self.default_model, self.api_mode,
                                  ...
                                  max_tokens_field=self.max_tokens_field, top_p=self.top_p,
